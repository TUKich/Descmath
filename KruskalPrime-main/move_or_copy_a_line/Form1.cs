...
/*
Цель программы
Алгоритм Крускала с минимальным связующим деревом и премиальный алгоритм минимального связующего дерева
Это для того, чтобы иметь возможность лучше понимать, как их применять.
+ - + - ++ - + - + - + - + - ЛОГИКА ГРАФИЗАЦИИ + - + - + - + - + - + - + - + - +
Пожалуйста, Прочтите Инструкции, Которые Написаны Здесь, Перед Запуском Программы...
• Когда программа запустится, вы можете добавить узел, щелкнув picturebox1 в левом нижнем углу.
• Сколько узлов вы создадите, вы должны сначала создать узлы, а затем нажать кнопку создать матрицу.
• Пожалуйста, нажмите кнопку создать матрицу после добавления такого количества узлов, какое количество узлов вы добавите.
• При нажатии кнопки создать матрицу создается матрица на странице tabpage1 объекта tabcontrol справа.
• Если вы нажмете левым щелчком мыши на кнопки в матрице, соединение между узлами будет установлено, и стоимость между узлами увеличивается один за другим с каждым щелчком, и вы увидите, что она увеличивается быстрее.
• Если вы нажмете правую кнопку мыши, длина-стоимость между узлами уменьшится. он уменьшается быстрее, если вы удерживаете его нажатым.
• Нажимая кнопки на матрице или нажимая узел и нажимая другой узел, вы устанавливаете соединение между узлами.
• Количество узлов ограничено 12, так как матрица может вместить столько же, сколько размер объекта tabepage1.
• Если вы создали graf, вы можете выбрать один из алгоритмов Kruskal или Prim и нажать кнопку run.
• Когда вы нажимаете кнопку "Выполнить", вы можете отслеживать, через какие узлы прошел алгоритм, и его общую потерю из раздела "Список затрат".
• Если созданная вами структура графика неверна или вы хотите создать новый график
Вы можете нажать кнопку создать новый график в левом нижнем углу.
• Если вы нажмете на объект checkbox1 с текстовым названием "сетка" в левом нижнем углу, вы измените фон picturebox1.
• При запуске любого из алгоритмов kruskal или premium в структуре графика происходят изменения цвета, и кнопка обновить используется для восстановления изменений цвета.
• Бонусный алгоритм начинается с 0 узла.
• Чтобы алгоритм Крускала работал должным образом, пожалуйста, установите соединение между всеми узлами. Установите соединения несвязанных узлов.
    ...
    public partial class Form1 : Form
    {
        ...
        private void Kruskal_minimum_spanning_tree()
        {
            ...
            while (edge_count < V - 1)
            {
                ...
                union1(a, b,parent);
                if(a!=-1||b!=-1)
                textBox2.Text += " Ребро " + edge_count + " : (" + a + ", " + b + ") затрат : " + min + " " + Environment.NewLine;
                Console.Write("Ребро {0}:({1}, {2}) затрат:{3} \n",
                    edge_count, a, b, min);
                edge_count++;
                mincost += min;
            ...
            textBox2.Text += " Мин затрат = " + mincost;
        }
        private void Prim_minimum_spanning_tree()
        {
            ...
            if(knkg==false)
            {
                MessageBox.Show("НАЧАЛЬНЫЙ УЗЕЛ ЯВЛЯЕТСЯ СЛУЧАЙНЫМ 'ПРИСОЕДИНЕН К 0' УЗЕЛ ДОЛЖЕН ИМЕТЬ СОЕДИНЕНИЕ С ДРУГИМИ УЗЛАМИ");
            }
            ...
            while (no_of_edges > 0)
            {
                ...
                no_of_edges--;
                visited[v] = 1;     
                textBox2.Text += " Ребро " + edge_count + " : (" + u + ", " + v + ") затрат : " + spanning[u, v]+""+ Environment.NewLine ;
                edge_count++;
            ...
            textBox2.Text += " Мин затрат= " + min_cost + "";
        }
        ...
        void button_click(object sender, EventArgs e)
        {
            ...
            if (tut != btn.Name && IsMouseDown == true)
            {
                ...
                if (matrix[k, k2] == 0)
                {
                    ...
                    {
                        MessageBox.Show("ПОЖАЛУЙСТА, НЕ ССЫЛАЙТЕСЬ НА ГРАФИК БЕЗ СОЗДАНИЯ МАТРИЦЫ" + es.ToString());
                    }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            label1.Text = "Алгоритм минимального связующего дерева";
        ...
        private void pictureBox1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("В этой программе максимальное количество узлов равно 12 !!!");
                nodecntrl = true;
